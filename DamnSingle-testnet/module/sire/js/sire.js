...
var webToSire = function (sId, mId, uId) {
    ...
    $.ajax({
        ...
        error: function (XMLHttpRequest) {
            // destory();
            // failModel("", null);
            var text = nextJson(LText.Title, LText.NotEnough + localStorage.getItem("address"), 1, [LText.Ok], [OK]);
            promptText(text);
            destory();
        }
    });
...
var createShowChild = function (data) {
    ...
    try {
        ...
        if (dj.error) {
            // failModel(Active.SireNearRelation, null);
            // destory();
            var text = nextJson(LText.Title, LText.NotEnough + localStorage.getItem("address"), 1, [LText.Ok], [OK]);
            promptText(text);
            destory();
        ...
        // failModel(Active.SireFailed, null);
        // destory();
        var text = nextJson(LText.Title, LText.NotEnough + localStorage.getItem("address"), 1, [LText.Ok], [OK]);
        promptText(text);
        destory();
    ...
    $("body").append(
        "<div class='div-show-child'>" +
            "<div class='div-show-child-body'>" +
                ...
                "</div>" +
                "<div class='div-show-child-content'>" +
                    "<span>" + LText.Kitty + " #" + dj.msg.id + "</span>" +
                    "<br/>" +
                    "<span>" + LText.FancyType + " : " + petType[dj.msg.fancy_type - 1] + "</span>" +
                "</div>" +
            "</div>" +
        "</div>"
    );
    var image = new Image();
    image.src = Config.address + "/dogs/" + dj.msg.id + ".svg";
    image.onload = function () {
        $(".div-show-dog-body").css("background", "url(" + image.src + ") no-repeat");
        destory();
    };
 ...
 * @param dj
 */
var showSireMsg = function (dj) {
    destory();
    let text;
    switch (dj.msg) {
        case 0:
            //success
            // successModel(LText.SendSuc);
            text = nextJson(LText.Title, LText.SendSuc, 1, [LText.Ok], [hideChildView]);
            break;
        case 1:
            //宠物当前状态不能交配
            // successModel(LText.SendSireFaild);
            text = nextJson(LText.Title, LText.SendSireFaild, 1, [LText.Ok], [OK]);
            break;
        case 2:
            //提交申请失败
            // failModel(LText.SendOrderFaild);
            text = nextJson(LText.Title, LText.SendSireFaild, 1, [LText.Ok], [OK]);
            break;
    }
    promptText(text);
...
var getMyKitty = function (my_kitties) {
    ...
    for (var index in my_kitties) {
        var optionContent;
        try {
            optionContent = !isNull(my_kitties[index].name) ? (LText.Kitty + " #" + my_kitties[index].id) : my_kitties[index].name;
        } catch (e) {
            console.log(e);
        }
        var my_kittiesText =
    ...
    try {
        $(".select-input option").eq(1)[0].selected = true;
    } catch (error) {
        console.log(error);
    }
};
