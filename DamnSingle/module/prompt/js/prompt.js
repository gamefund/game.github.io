...
var promptText = function (nextJson) {
    ...
    var promptText =
        '<div class="prompt-main">' +
            '<div class="prompt-body">' +
                '<div class="prompt-title prompt-text" style="height:8vh">' + nextJson.title + '</div>' +
                '<div class="prompt-msg">' +
                    '<div class="prompt-msg-body">' + nextJson.msg + '</div>'
                '</div>' +
            '</div>' +
        '</div>';
    ...
    for (var i = 0; i < nextJson.bottonNum; i++) {
        $('.prompt-msg').after(
            '<div class="prompt-next' + i + ' prompt-text " style="color:#168AEF;">' +
                (isNull(nextJson.bottonMsg) ? nextJson.bottonMsg[i] : "OK") +
            '</div>');
        $('.prompt-next' + i + '').attr("nextWay", i);
        $('.prompt-next' + i + '').css("height", "7vh");
        $('.prompt-next' + i + '').css("line-height", "7vh");
