...
var myKittyPageLoad = function (params) {
    ...
    if (isNull(userId)) {
        if (kittyOwnerId != userId && isNull(kittyOwnerId)) {    //进入别人的猫界面
            // $('.loginOut').show();
            // $(".setting-a").show();
            // $(".invite-friend").show();
            $(".showId").css("left", "44vw");
        } else if (userId == kittyOwnerId || !isNull(kittyOwnerId)) {    //进入的是我的猫界面
            // $(".invite-friend").show();
            $('.loginOut').show();
            $(".setting-a").hide();
        }
...
var getUserInfo = function (kittyOwnerId) {
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            fillInUserInfo(response.msg, uid);
        },
...
var fillInUserInfo = function (data, uid) {
...
var fastCheck = function () {
    ...
    //注销登入
    $('.loginOut').bind("click", logoutFunc);
    ...
    for (var i = 0; i < data.kitties.length; i++) {
        ...
        if (data.kitties[i].status.is_ready) { //判断狗状态， ready--sire\sale; noReady--reste/gestate
            ...
            if (!data.kitties[i].status.is_gestating) {
                if (!isNull(data.kitties[i].left_cooldown)) {
                    isLock = true;
                }
